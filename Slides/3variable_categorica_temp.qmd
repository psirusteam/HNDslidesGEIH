...
```{r, prop_ZonaH_edad, eval=TRUE, echo=FALSE}
```

## Proporción de mujeres en la zona urbana y rural

```{r,  prop_ZonaM_edad, eval=FALSE, echo=TRUE}
...
```{r, prop_ZonaM_edad, eval=TRUE, echo=FALSE}
```

## Proporción de hombres en la area  disponible para trabajar

```{r, prop_ZonaH_disponible, eval=FALSE,echo=TRUE}
...
```{r, prop_ZonaH_disponible, eval=TRUE, echo=FALSE}
```

## Proporción de mujeres en la área urbana y rural

```{r,prop_ZonaM_disponible, eval=FALSE}
...
```{r,prop_ZonaM_disponible, eval=TRUE, echo=FALSE}
```
